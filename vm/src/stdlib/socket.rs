...
pub fn make_module(vm: &VirtualMachine) -> PyObjectRef {
    ...
    let module = py_module!(vm, "_socket", {
        ...
        "AF_IPX" => ctx.new_int(c::AF_IPX),
        "AF_APPLETALK" => ctx.new_int(c::AF_APPLETALK),
        "AF_NETROM" => ctx.new_int(c::AF_NETROM),
        "AF_BRIDGE" => ctx.new_int(c::AF_BRIDGE),
        "AF_ATMPVC" => ctx.new_int(c::AF_ATMPVC),
        "AF_X25" => ctx.new_int(c::AF_X25),
        "AF_INET6" => ctx.new_int(c::AF_INET6),
        "AF_ROSE" => ctx.new_int(c::AF_ROSE),
        "AF_DECnet" => ctx.new_int(c::AF_DECnet),
        "AF_NETBEUI" => ctx.new_int(c::AF_NETBEUI),
        "AF_SECURITY" => ctx.new_int(c::AF_SECURITY),
        "AF_KEY" => ctx.new_int(c::AF_KEY),
        "AF_NETLINK" => ctx.new_int(c::AF_NETLINK),
        "AF_ROUTE" => ctx.new_int(c::AF_ROUTE),
        "AF_PACKET" => ctx.new_int(c::AF_PACKET),
        "AF_ASH" => ctx.new_int(c::AF_ASH),
        "AF_ECONET" => ctx.new_int(c::AF_ECONET),
        "AF_ATMSVC" => ctx.new_int(c::AF_ATMSVC),
        "AF_RDS" => ctx.new_int(c::AF_RDS),
        "AF_SNA" => ctx.new_int(c::AF_SNA),
        "AF_IRDA" => ctx.new_int(c::AF_IRDA),
        "AF_PPPOX" => ctx.new_int(c::AF_PPPOX),
        "AF_WANPIPE" => ctx.new_int(c::AF_WANPIPE),
        "AF_LLC" => ctx.new_int(c::AF_LLC),
        "AF_CAN" => ctx.new_int(c::AF_CAN),
        "AF_TIPC" => ctx.new_int(c::AF_TIPC),
